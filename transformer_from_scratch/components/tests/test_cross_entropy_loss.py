...
def test_where_logits_are_fully_accurate():
    ...
    inputs: BatchTokenIndices = sample_tokens[:, :-1]  # 1,2,3
    ...
    logits: BatchLogits = F.one_hot(sample_tokens[:, 1:]).float() * 999999  # 2,3,4
...
def test_loss_with_random_logits():
    ...
    logits: BatchLogits = torch.randn(1, 3, 4)
...
def test_loss_with_uniform_logits():
    ...
    logits: BatchLogits = torch.zeros(1, 3, 4)
